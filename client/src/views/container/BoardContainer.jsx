...
import { compose, chooseNumber, getTools, getNumbers, checkDuplicates, parseData } from '../../util/helpers'
...
function BoardContainer() {
    ...
    const { getState, dispatcher } = useStore(),
        { current } = getState?.(),
        { current: { mapped: { render, selected, history: previous }, complete, playing, completed, matrix } } = getState?.(),
        { _id, diff } = useParams(),
        memoID = useMemo(() => _id, [_id])

    const isFinished = useMemo(() => render.map(({ value }) => value).filter(item => !!item).map(item => item.toString()).join(''), [render]),
    ...
    useEffect(() => { if (new RegExp(`^${isFinished}$`).test(isCompleted)) setWinner(true) }, [isFinished, isCompleted])
    ...
    useEffect(() => {
        ...
        if (!playing && !completed) Request.updatePlaying(memoID).catch(console.error)
    ...
    const handlePencil = () => setPencil(prev => ({
        ...prev,
